...
export default function Products() {
  ...
  const filtered = active === "All"
    ? products
    : products.filter((p) => p.category === active || p.level === active.toLowerCase());
      ...
      <section className="relative overflow-hidden bg-[#061B33] py-24 lg:py-28">
        ...
        <div className="relative max-w-4xl mx-auto px-6 text-center anim-fadeup">
          ...
          <p className="mt-6 text-lg text-slate-300 max-w-2xl mx-auto leading-relaxed">
            High-quality robotics kits, AI tools, IoT experiment kits, and educational electronics for school labs.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-24 text-slate-400">
              ...
              <p className="text-sm mt-2">Add products from the Admin Dashboard to display them here.</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-20 text-slate-500">No products in this category.</div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {filtered.map((p) => (
                <Link
                ...
                >
                  {p.imageUrl ? (
                    ...
                    <div className="w-12 h-12 rounded-xl bg-[#0b2545] flex items-center justify-center mb-4 group-hover:bg-cyan-500 transition-colors duration-300">
                      <Package size={22} className="text-cyan-300 group-hover:text-white transition-colors duration-300" />
                    </div>
                  )}
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed mb-3 line-clamp-2">{p.description || p.desc}</p>
                  <div className="text-sm font-semibold text-cyan-600">
                    {p.price ? `₹${p.price}` : "Contact for Price"}
                  </div>
                </Link>
              ))}
            </div>
