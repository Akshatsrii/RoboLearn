 ...
 *   columns    – [{ key, label }]                 → table columns
 *   fields     – [{ name, label, type, options }]  → add/edit form fields
 *                type: "text" | "textarea" | "number" | "select" | "checkbox"
 *   emptyItem  – default values for the "Add new" form
 */
    ...
    <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr><td colSpan={columns.length + 1} className="px-5 py-12 text-center text-slate-400">
                  ...
                  <tr key={item._id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => openEdit(item)} className="p-2 rounded-lg text-slate-500 hover:bg-cyan-50 hover:text-cyan-600 transition-colors">
                          <Pencil size={15} />
                        </button>
                        <button onClick={() => handleDelete(item._id)} className="p-2 rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600 transition-colors">
                          <Trash2 size={15} />
                        </button>
                      </div>
                    </td>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-[#061B33]/50 backdrop-blur-sm" onClick={() => setModalOpen(false)} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white">
              <h2 className="font-bold text-[#0b2545]">{editing ? "Edit" : "Add New"} {title.replace(/s$/, "")}</h2>
            ...
            <form onSubmit={handleSave} className="px-6 py-5 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 bg-[#0b2545] hover:bg-cyan-600 disabled:opacity-60 text-white py-2.5 rounded-xl text-sm font-semibold transition-colors"
                >
                  {saving ? "Saving..." : "Save"}
                </button>
              </div>
